...
class TorchProcessor(BaseProcessor):
    ...
    def _load_model(self):
        ...
        # model_path = os.path.join(self.model_dir, self.model_file + '.pth')
        # if not os.path.exists(model_path):
        #     raise FileNotFoundError(f"모델 파일을 찾을 수 없습니다: {model_path}")

        # map_location을 사용하여 모델을 CPU로 로드합니다.
        # self.model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
        self.model_loaded = True
        # # if not self.model_loaded:
        # self.model_loaded = True
        # self.model.load_state_dict(torch.load(self.model_file + '.pth'),map_location=torch.device('cpu'))
