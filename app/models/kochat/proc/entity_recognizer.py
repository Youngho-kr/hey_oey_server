...
class EntityRecognizer(TorchProcessor):
    ...
    def predict(self, sequence: Tensor) -> list:
        ...
        # print("predicts : ", predicts.tolist())
        ...
        # 디버깅: 예측된 라벨 인덱스 출력
        # print("Predicted labels (indices):", predicts.tolist())

        # 디버깅: 라벨 딕셔너리 출력
        # print("Label dictionary:", self.label_dict)

        # 리스트 컴프리헨션을 사용하여 라벨 추출
        entities = [list(self.label_dict.keys())[list(self.label_dict.values()).index(label.item())] for label in
                    predicts]

        # 엔티티 시퀀스에서 길이만큼 자르기
        entities = entities[:length]

        # print("entity recognizer predict : ", entities)
        return entities
    ...
    def _forward(self, feats: Tensor, labels: Tensor = None, length: Tensor = None):
        ...
        if isinstance(self.loss, CRFLoss):
            ...
            # print("isinstance predicts :", predicts.tolist())
        else:
